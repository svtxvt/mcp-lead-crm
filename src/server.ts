...
export function createServer(store: Store, webhookUrl = process.env.N8N_WEBHOOK_URL) {
  const server = new McpServer({ name: "mcp-lead-crm", version: "0.2.1" });
  const pending = new Set<Promise<unknown>>();
  let stopping = false;

  async function run<T>(operation: () => Promise<T>, message = "CRM operation failed"): Promise<T> {
  ...
  }, async (input) => run(async () => text(await store.addLead(input))));
  ...
  }, async ({ id, score, reason }) => run(async () => text(await store.qualifyLead(id, score, reason))));
  ...
  }, async ({ id, stage, note }) => run(async () => text(await store.moveStage(id, stage, note))));
  ...
  }, async ({ id, type, note, due_at }) => run(async () => text(await store.logActivity(id, type, note, due_at))));
  ...
  }, async ({ stage, limit }) => run(async () => text(pipelineTable(await store.listLeads(stage, limit)))));
  ...
  }, async ({ days }) => run(async () => text(await store.dueFollowups(days))));
  ...
  }, async ({ query }) => run(async () => text(await store.searchLeads(query))));
  ...
  }, async ({ path }) => run(async () => text(await store.importCsv(path))));

  server.registerTool("export_csv", {
    description: "Export all leads to a CSV file inside the CRM database directory.",
    inputSchema: z.object({ path: z.string().trim().min(1) }),
  }, async ({ path }) => run(async () => text({ path, exported: await store.exportCsv(path) })));
  ...
  server.registerTool("trigger_n8n", {
  ...
  }, async ({ event, lead_id, payload }, context) => run(async () => {
    ...
    if (!["http:", "https:"].includes(url.protocol) || url.username || url.password) {
      throw new Error("N8N_WEBHOOK_URL must be HTTP(S) without credentials");
    }
    const response = await fetch(url, {
      ...
      redirect: "error",
      signal: AbortSignal.any([context.mcpReq.signal, AbortSignal.timeout(10_000)]),
    });
    if (!response.ok) throw new Error(`HTTP ${response.status} ${response.statusText}`);
    await response.text();
    if (lead_id) await store.logActivity(lead_id, "n8n:trigger", JSON.stringify(body));
    return text({ dry_run: false, status: response.status });
  }, "n8n webhook failed"));
  ...
  server.registerResource("lead", new ResourceTemplate("crm://lead/{id}", { list: undefined }), {
    ...
    mimeType: "application/json",
  }, async (uri, { id }) => run(async () => {
    const lead = await store.getLead(String(id));
    if (!lead) throw new ResourceNotFoundError(uri.href, "Lead resource not found");
